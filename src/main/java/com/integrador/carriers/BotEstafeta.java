...
import com.estafeta.bot.EstafetaBot;
...
public class BotEstafeta implements Carrier{
	...
	public Object executeWebService() throws Exception {
		...
		EstafetaBot bot = new EstafetaBot(values.get("url").toString().trim(), values.get("usuario").toString().trim(), values.get("password").toString().trim());
		EstafetaBot botToken = new EstafetaBot(values.get("url").toString().trim(), values.get("usuario_bot").toString().trim(), values.get("password_bot").toString().trim());
		com.app.models.Token token = botToken.doSignIn();
		if(token!=null) {
			com.estafeta.bot.Label label = new com.estafeta.bot.Label();
			...
			com.estafeta.bot.Credential credentials = new com.estafeta.bot.Credential();
			credentials.setUsername(bot.getUser());
			credentials.setPassword(bot.getPassword());
